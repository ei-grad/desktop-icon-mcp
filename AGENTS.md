...
- This repo contains a desktop-icons MCP server. The MCP entrypoint is `bin/desktop-icon-mcp.js`; Windows Explorer/ListView operations go through `scripts/desktop_icon_helper.ps1`.
...
- Use `src/planner.js` for deterministic layout planning. `optimize_desktop_islands.js` is a CLI/module wrapper around that planner.
